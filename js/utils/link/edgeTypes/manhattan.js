...
function getRadiusPath(pointArr) {
  ...
  if (
    pointArr[0].x === pointArr[pointArr.length - 1].x ||
    pointArr[0].y === pointArr[pointArr.length - 1].y
  ) {
    path = [
      'M', pointArr[0].x, pointArr[0].y,
      'L', pointArr[pointArr.length - 1].x, pointArr[pointArr.length - 1].y
    ].join(' ');
    return {
      path,
